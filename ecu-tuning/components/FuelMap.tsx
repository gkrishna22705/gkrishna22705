...
interface FuelMapProps {
  fuelMap: number[][]
  setFuelMap: (map: number[][]) => void
}

export default function FuelMap({ fuelMap, setFuelMap }: FuelMapProps) {
  ...
  const getCellColor = (value: number) => {
    if (value < 12) return 'bg-blue-600'
    if (value < 13.5) return 'bg-cyan-600'
    if (value < 14.7) return 'bg-green-600'
    if (value < 15.5) return 'bg-yellow-600'
    if (value < 16.5) return 'bg-orange-600'
    return 'bg-red-600'
  }

  const adjustMap = (amount: number) => {
    const newMap = fuelMap.map(row => row.map(cell => 
      Math.max(10, Math.min(20, cell + amount))
    ))
    setFuelMap(newMap)
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gray-800/50 backdrop-blur-sm rounded-xl p-6 border border-gray-700">
        <h2 className="text-xl font-bold text-white mb-4">Fuel Map Editor</h2>
        <div className="flex flex-wrap gap-4">
          <button
            onClick={() => adjustMap(-0.1)}
          ...
          <button
            onClick={() => setFuelMap(Array(16).fill(0).map(() => Array(16).fill(14.7)))}
            className="px-4 py-2 bg-gray-600 hover:bg-gray-700 text-white rounded-lg transition-colors"
          >
            Reset to Stoich (14.7)
          </button>
          <div className="flex items-center space-x-2 ml-auto">
            <span className="text-gray-400 text-sm">AFR Range:</span>
            <div className="flex space-x-1">
              <div className="w-4 h-4 bg-blue-600 rounded"></div>
              <span className="text-xs text-gray-400">Rich</span>
              <div className="w-4 h-4 bg-green-600 rounded ml-2"></div>
              <span className="text-xs text-gray-400">Stoich</span>
              <div className="w-4 h-4 bg-red-600 rounded ml-2"></div>
              <span className="text-xs text-gray-400">Lean</span>
            </div>
          </div>
        </div>
      </div>
